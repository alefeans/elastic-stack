...
O Logstash tem a função básica de processar dados de diferentes fontes, fazer a transformação desses dados (ou não), e então fazer o envio para algum "armazém" de dados, como o Elasticsearch por exemplo. Como fonte de dados vamos utilizar o _Apache HTTP server_, sendo assim faça a instalação de acordo com a sua distribuição Linux (pesquisa ai, deixa de ser preguiçoso). Como estou utilizando o Fedora:
